...
if dbname.startswith('SIFT'):
    ...
    xb = mmap_fvecs('siftsmall/siftsmall_base.fvecs')
    xq = mmap_fvecs('siftsmall/siftsmall_query.fvecs')
    xt = mmap_fvecs('siftsmall/siftsmall_learn.fvecs')
    # deep1B's train is is outrageously big
    # xt = xt[:10 * 1000 * 1000]
    xb = xb[:5 * 1000 * 1000]
...
def get_populated_index():
    ...
    if not os.path.exists(filename):
        ...
        i0 = 0
        t0 = time.time()
        for xs in matrix_slice_iterator(xb, 10):
            i1 = i0 + xs.shape[0]
...
ps = faiss.ParameterSpace()     #对建索引的相关配置重新设置，如编码方式，降维方式.
ps.initialize(index)
